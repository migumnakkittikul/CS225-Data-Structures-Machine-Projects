...
Animation FloodFilledImage::animate(unsigned frameInterval) const {
  ...
  Animation animation;
  unsigned i = 0;
  while (i < traversals_.size()){
    unsigned count = 0;
    auto img = png_;
    auto trav_begi = traversals_.begin();
    auto color_begi = cpick_.begin();
    auto trav = *trav_begi;
    auto cpick = *color_begi;
    auto anime = trav->begin();
    
    while (anime != trav->end()){
      if (count % frameInterval == 0){
        animation.addFrame(img);
      }
      Point p = *anime;
      HSLAPixel & pixel = img.getPixel(p.x, p.y);
      pixel = cpick->getColor(p.x, p.y);
