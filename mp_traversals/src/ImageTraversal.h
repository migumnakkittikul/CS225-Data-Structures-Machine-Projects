  ...
  */
  ...
  public:
    ...
    */
    ...
    public:
      Iterator();
      Iterator(const PNG & png, const Point & start, double tolerance, ImageTraversal *traversal_);
    ...
    private:
      ...
      //bool cantraverse(const Point &p);
      bool cantraverse(const Point &p){
        if ((p.x >= traversal_ -> png_.width()) || (p.y >= traversal_ -> png_.height()) || (visited_[p.x][p.y] == true)){
          return false;
        }
        auto tolenum = calculateDelta(traversal_ -> png_.getPixel(p.x, p.y), traversal_ -> png_.getPixel(traversal_ -> start_.x, traversal_ -> start_.y));
        if ((traversal_ -> tolerance_) <= tolenum){
          return false;
        }
          return true;
 }
