...
void DHHashTable<K, V>::insert(K const& key, V const& value)
{
    ...
    if ((elems + 1) / static_cast<double>(size) >= 0.7) {
        resizeTable();
    }
    ...
    for (; table[idx] != NULL && table[idx]->first != key; ++i, idx = (h + jump * i) % size) {
}
...
int DHHashTable<K, V>::findIndex(const K& key) const
{
    ...
    for (; table[idx] != NULL && table[idx]->first != key; idx = (h + jump * i) % size) {
    ++i;
    if (i == size) {
        return -1;
    }
}
    return (table[idx] == NULL) ? -1 : idx;
