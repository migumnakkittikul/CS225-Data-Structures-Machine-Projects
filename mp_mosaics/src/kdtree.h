...
class KDTree
{
  ...
  public:
    ...
    KDTreeNode* kdtreebuild(RandIter start, RandIter end, int dimen);
     ...
     */
    //template <int Dim>
    KDTreeNode* copytreebuild(const KDTreeNode* othertnode, int dimen);
    ...
    //template <int Dim>
    void cleartree(KDTreeNode* tnode);
    ~KDTree();
     ...
     */
    KDTreeNode* nearesthelper(const Point<Dim>& point, KDTreeNode* rootnode, int dimen) const;
...
RandIter parti(RandIter begin, RandIter end, RandIter pivotpoint, Comparator cmp);
