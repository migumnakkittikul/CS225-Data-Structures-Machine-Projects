...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while (singleQueue.size() + mergeQueue.size() > 1) {
        ...
        int combinedFreq = left->freq.getFrequency() + right->freq.getFrequency();
        TreeNode* merged = new TreeNode(combinedFreq);
        merged->left = left;
        merged->right = right;
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    bool isLeafNode = bfile.getNextBit();
    if (isLeafNode) {
        char character = bfile.getNextByte();
        TreeNode* leafnode = new TreeNode(Frequency(character, 0));
        return leafnode;
    } 
    else {
